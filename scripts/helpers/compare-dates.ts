 ...
 */
export function isDueInTimeFrame(dueDate: string, timeFrame: string): boolean {
  ...
  const due = new Date();
  due.setFullYear(parseInt(year), parseInt(month) - 1, parseInt(day));
...
export function isThisYear(due: Date, today: Date) {
  return due.getFullYear() === today.getFullYear();
}

function isToday(due: Date, today: Date) {
  return due.toDateString() === today.toDateString();
}

function isThisWeek(due: Date, today: Date) {
...
function isThisMonth(due: Date, today: Date) {
  return (
