 ...
 */
export function updateProjectsArray(array: Project[]) {
  projectsArray = array;
 ...
 */
function deleteTaskOrProject(cardId: string, project: string) {
  ...
  let arrayJSON: string | null = localStorage.getItem(cardType + "sArray");

  // Check if array is not null.
  if (arrayJSON === null) arrayJSON = "[]";

  // Parse the retrieved array into an array.
...
export function deleteIconFunctionality() {
  ...
  deleteIcons.forEach(icon => {
    icon.addEventListener("click", event => {
      ...
      const fromProject = target.getAttribute("data-project");
      ...
      if (
        ...
        deleteTaskOrProject(cardId === null ? "" : cardId, type);
        ...
        if (fromProject === "project") {
          ...
          const taskCard: HTMLElement | null = document.getElementById(
            `${cardId}`
          );
          if (taskCard) {
            taskCard.remove();
